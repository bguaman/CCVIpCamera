...
{
	//set the title
	ofSetWindowTitle(" Community Core Vision for Interactive Proyection - ESPOL-CTI 2011");
...
void ofxNCoreVision::loadXMLSettings()
{
	...
	CONSTANT				    = XML.getValue("CONFIG:FLOAT:CONSTANTE",0.005f);
	//Tracking Options
...
void ofxNCoreVision::saveSettings()
{
	...
	//CTI-ESPOL-BG
	//ponemos true al vlaor de centerpoint por default
//	XML.setValue("CONFIG:BOOLEAN:OUTLINES",);
...
void ofxNCoreVision::_update(ofEventArgs &e)
{
	...
	else//else process camera frame
	{
		...
		if (bGPUMode)
		{
            ...
            contourFinder.valorconstante = CONSTANT;
		...
		{
			tracker.track(&contourFinder,contourFinder.bCenterPoint);
		}
		...
		if (myTUIO.bOSCMode || myTUIO.bTCPMode || myTUIO.bBinaryMode)
		{
			...
			myTUIO.setMode(contourFinder.bTrackFingers , contourFinder.bTrackObjects, contourFinder.bTrackFiducials , contourFinder.bCenterPoint , contourFinder.bPolygon);
...
void ofxNCoreVision::drawFingerOutlines()
{
	...
	if(contourFinder.bTrackFingers)
	{
		for (int i=0; i<contourFinder.nBlobs; i++)
		{
			...
			//CTI-ESPOL-BG
			//Preguntamos si queremos  o centerpoints en el contorno
	...
	if(contourFinder.bTrackObjects)
	{   //CTI-ESPOL-Bg
	    //un print para saber si entro en esta porcion de codigo
	   // printf("aqui entro en objects");
