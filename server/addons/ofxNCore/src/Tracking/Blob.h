...
class Blob {
 ...
 ofCircle(x + centroid.x/inputWidth * outputWidth, y + centroid.y/(inputHeight) * outputHeight,2.0f);
         //  ofBeginShape();
        ...
        void drawContoursRounded(float x = 0, float y = 0, float inputWidth = ofGetWidth(), float inputHeight = ofGetHeight(), float outputWidth = ofGetWidth(), float outputHeight = ofGetHeight())
		{
            glPushMatrix();
                glTranslatef(x + angleBoundingRect.x/inputWidth * outputWidth, y + angleBoundingRect.y/inputHeight * outputHeight, 0.0f);
                glRotatef(-angle, 0.0f, 0.0f, 1.0f);
            ...
            for (int i = 0; i < nPts; i++){
                   ofVertex(x + pts[i].x/inputWidth * outputWidth, y + pts[i].y/(inputHeight) * outputHeight);
                   //CTI-BG
                   //le decimos que dibuje circulos por cada 10 puntos
                  // if((i%10)==0){
                    ofCircle(x + pts[i].x/inputWidth * outputWidth, y + pts[i].y/(inputHeight) * outputHeight,2.0f);
                    // ofCircle(x + pts[i].x/inputWidth * outputWidth, y + pts[i].y/(inputHeight) * outputHeight,2.0f);
                   //}
		}
        ...
        //CTI-ESPOL-BG
        //funciones para elegir el tipo de blob con centroide o con contornos
        void drawContoursCenter(float x = 0, float y = 0, float inputWidth = ofGetWidth(), float inputHeight = ofGetHeight(), float outputWidth = ofGetWidth(), float outputHeight = ofGetHeight())
		{
            glPushMatrix();
                glTranslatef(x + angleBoundingRect.x/inputWidth * outputWidth, y + angleBoundingRect.y/inputHeight * outputHeight, 0.0f);
                glRotatef(-angle, 0.0f, 0.0f, 1.0f);
