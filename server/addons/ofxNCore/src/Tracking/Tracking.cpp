...
void BlobTracker::track(ContourFinder* newBlobs,bool bCentered)
{
/*********************************************************************
//Object tracking
*********************************************************************/
...
****************************************************************************/
	...
	{
	 //    cout<<"TRACKING BLOB STEP1:("<<trackedBlobs.at(i).centroid.x<<","<<trackedBlobs.at(i).centroid.y<<")"<<endl;
		...
		if(winner == -1) //track has died, mark it for deletion
		{
			...
			//erase calibrated blob from map
				if(bCentered){
			for (int m=0;m<TouchEvents.messenger.puntos.size();m++){
			calibrate->cameraToScreenPosition(TouchEvents.messenger.puntos.at(m).x, TouchEvents.messenger.puntos.at(m).y);
		...
		else //living, so update it's data
		{
			for(int j = 0; j < newBlobs->nBlobs; j++)
			{
				if(trackedBlobs[i].id == newBlobs->blobs[j].id)
				{
					...
					//  cout<<"TRACKING BLOB STEP3:("<<trackedBlobs[i].centroid.x<<","<<trackedBlobs[i].centroid.y<<")"<<endl;
...
//            cout<<"TRACKING BLOB STEP4:("<<newBlobs->blobs[i].centroid.x<<","<<newBlobs->blobs[i].centroid.y<<")"<<endl;
           //  cout<<"TRACKING BLOB:("<<TouchEvents.messenger.centroid.x<<","<<TouchEvents.messenger.centroid.y<<")"<<endl;
