...
void ofxNCoreVision::setupControls()
{
	...
	//BGUAMAN
	//ANADIR POLYGON BUTTON FOR PAPER
	trackingPanel->addButton(appPtr->trackingPanel_trackPolygon, "Polygon", OFXGUI_BUTTON_HEIGHT, OFXGUI_BUTTON_HEIGHT, kofxGui_Button_On, kofxGui_Button_Switch);
	//---------------------------------
	trackingPanel->mObjHeight = 120;
	...
	//CTI-ESPOL-BG
	trackPanel->addButton(appPtr->trackedPanel_CenterPoints, "Center", OFXGUI_BUTTON_HEIGHT, OFXGUI_BUTTON_HEIGHT, kofxGui_Button_Off, kofxGui_Button_Switch);
	trackPanel->addButton(appPtr->trackedPanel_RoundedPoints, "Rounded", OFXGUI_BUTTON_HEIGHT, OFXGUI_BUTTON_HEIGHT, kofxGui_Button_On, kofxGui_Button_Switch);

    trackPanel->addSlider(appPtr->trackedPanel_Dpconstant, "DP. Constant", 140, 13, 0.005f, 0.05f, 0.03, kofxGui_Display_Float4, 0);
    trackPanel->addButton(appPtr->trackedPanel_Polygon, "PolyMode", OFXGUI_BUTTON_HEIGHT, OFXGUI_BUTTON_HEIGHT, kofxGui_Button_Off, kofxGui_Button_Switch);
	...
	//---------------------
	trackPanel->mObjects[5]->mObjX = 90;
	trackPanel->mObjects[5]->mObjY = 100;
	trackPanel->mObjects[6]->mObjX = 165;
	trackPanel->mObjects[6]->mObjY = 100;
	trackPanel->mObjects[7]->mObjX = 185;
	trackPanel->mObjects[7]->mObjY = 100;
    trackPanel->mObjects[8]->mObjX = 90;
	trackPanel->mObjects[8]->mObjY = 110;
	//-----------------------
	...
	****************************/
	...
	controls->update(appPtr->trackedPanel_Dpconstant, kofxGui_Set_Bool, &appPtr->CONSTANT, sizeof(float));
	//Template Area
...
// 			break;
		...
		//CTI-ESPOL-BG
			case trackedPanel_CenterPoints:
			if(length == sizeof(bool)){
				contourFinder.bCenterPoint = *(bool*)data;
        	if(contourFinder.bCenterPoint==true){
        	    contourFinder.bRoundedPoints=false;
				    cout << contourFinder.bCenterPoint << endl;

                    	controls->update(appPtr->trackedPanel_RoundedPoints, kofxGui_Set_Bool, (&appPtr->contourFinder.bRoundedPoints), sizeof(bool));
				}else{
               contourFinder.bRoundedPoints=true;
				controls->update(appPtr->trackedPanel_RoundedPoints, kofxGui_Set_Bool, (&appPtr->contourFinder.bRoundedPoints), sizeof(bool));
				}
			...
			break;
			case trackedPanel_RoundedPoints:
			if(length == sizeof(bool)){


				contourFinder.bRoundedPoints = *(bool*)data;
				//BGUAMAN
				//andimos como dpconstant -1


				if(contourFinder.bRoundedPoints==true){
				    contourFinder.bCenterPoint=false;
				    cout << contourFinder.bRoundedPoints << endl;

                    	controls->update(appPtr->trackedPanel_CenterPoints, kofxGui_Set_Bool, (&appPtr->contourFinder.bCenterPoint), sizeof(bool));
				}else{
                    contourFinder.bCenterPoint=true;
				controls->update(appPtr->trackedPanel_CenterPoints, kofxGui_Set_Bool, (&appPtr->contourFinder.bCenterPoint), sizeof(bool));
				}
				//bcenterPoint=false;

				//printf(broundedPoints);

			}
			break;
            case trackedPanel_Polygon:
			if(length == sizeof(bool)){


				contourFinder.bPolygon = *(bool*)data;
				//BGUAMAN
				//andimos como dpconstant -1


                    	controls->update(appPtr->trackedPanel_Polygon, kofxGui_Set_Bool, (&appPtr->contourFinder.bPolygon), sizeof(bool));
			...
			//BGUAMAN
			//----------------------------------------------
			case trackedPanel_Dpconstant:
			if(length == sizeof(float))
			{
				CONSTANT = *(float*)data;
                if (CONSTANT < 0.01)
                CONSTANT= 0.005f;
                else if(CONSTANT >= 0.01 && CONSTANT < 0.015)
                CONSTANT= 0.010f;
                else if(CONSTANT >= 0.015 && CONSTANT < 0.02)
                CONSTANT = 0.015f;
                else if(CONSTANT >= 0.02 && CONSTANT < 0.025)
                CONSTANT = 0.02f;
                else if(CONSTANT >= 0.025 && CONSTANT < 0.03)
                CONSTANT = 0.025f;
                else if(CONSTANT >= 0.03 && CONSTANT < 0.035)
                CONSTANT = 0.03f;
                else if(CONSTANT >= 0.035 && CONSTANT < 0.04)
                CONSTANT = 0.035f;
                else if(CONSTANT >= 0.04 && CONSTANT < 0.045)
                CONSTANT = 0.04f;
                else if(CONSTANT >= 0.045 && CONSTANT < 0.05)
                CONSTANT = 0.045f;
                else if(CONSTANT >= 0.05)
                CONSTANT = 0.05f;

                  cout <<"cionstanteGUI"<<CONSTANT<<endl;
			}
			break;
