...
//BG
//aniadimos un parametro al final para parametrizar via interfaz grafica la manera en la que enviamos los blobs
void TUIO::setMode(bool fingers, bool objects, bool fiducials , bool centered, bool poly)
{
	bFingers = fingers;
	bObjects = objects;
...
void TUIO::sendTUIO(std::map<int, Blob> * fingerBlobs, std::map<int, Blob> * objectBlobs ,std::list <ofxFiducial> * fiducialsList)
{
	...
	if(bOSCMode)
	{
	if(bFingers && bCentered && bPoly)
		{
			ofxOscBundle b;
			ofxOscMessage alive;
			// Sends alive message - saying 'Hey, there's no alive blobs'
			alive.setAddress("/tuio/2Dblb");
			alive.addStringArg("alive");

			// Send fseq message
			ofxOscMessage fseq;
			fseq.setAddress( "/tuio/2Dblb" );
			...
			else // actually send the blobs
			{
              if(bFingers && bCentered & bPoly){
                        ...
                        for(blob = fingerBlobs->begin(); blob != fingerBlobs->end(); blob++){
                           ...
                           /* int eachpoint=floor(double(blob->second.nPts)/5);
                            int sumador =0;
                            ofPoint ptempa;
*/
                        ...
                        //    ptempa=blob->second.pts[sumador];

                          //  sumador=sumador+eachpoint;
                            ofPoint ptempa=blob->second.puntos.at(m);
                            float xc=ptempa.x;
                            float yc=ptempa.y;
                            set.addFloatArg(ptempa.x);            // dX
                            set.addFloatArg(ptempa.y);
       ...
       //if(bFingers && bCentered==false)
...

		{
			...
			else // actually send the blobs
			{
				...
				if(bFingers)
				{
					...
					//CTI - BG
					// Aqui ponemos un contador que va a limitar el numero de blobs que se envien a 23 ( con 24 se cae el eclipse )
					//int cont=0;
					for(blob = fingerBlobs->begin(); blob != fingerBlobs->end() ; blob++)

					{
					    ...
					    //CTI-ESPOL-BG
					    //aqui desglosamos los puntos del blob y hacemos que envie cada punto como un blob en si
					     //vector <ofPoint>  tempoints=blob->second.pts;
		...
		if(bObjects)
		{ //CTI-BG
		    //comentamos esta funcion porque no estamos interesados en enviar objetos
		    /*
		...
		if(bFiducials)
		{//CTI-BG
		    //comentamos esta funcion porque no estamos interesados en enviar fiducials
